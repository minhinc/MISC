...
#  self.wdgt.lwcity.populate(self.db.get('city','name'))
  self.wdgt.lwcountry.populate(self.db.get('country','name'))
  self.wdgt.show()
  fetchc.handle(self)
 def get(self):
  companyname=None
  matchobj=None
  if not len(self.wdgt.lwtech.lwt.curselection()) or not len(self.wdgt.lwcity.lwt.curselection()) or not len(self.wdgt.lwcountry.lwt.curselection()):
   ...
   for line in open(self.wdgt.filename):
    ...
    if(not matchobj):
     ...
     for email in [ email for email in line.split() if re.match(r'(\b[A-Za-z0-9._%-]+\@\w+[.](?:\w+[.]?)*\b)',email,re.I) ]:
      self.db.fill('track',((email.lower(),self.db.get('company','id','name',companyname)[0][0], self.db.get('tech','id','name',self.wdgt.lwtech.lwt.get(self.wdgt.lwtech.lwt.curselection()[0]))[0][0], self.db.get('city','id','name',self.wdgt.lwcity.lwt.get(self.wdgt.lwcity.lwt.curselection()[0]))[0][0], self.db.get('country','id','name',self.wdgt.lwcountry.lwt.get(self.wdgt.lwcountry.lwt.curselection()[0]))[0][0], int(re.sub('-','',datetime.date.today().isoformat()))),))
