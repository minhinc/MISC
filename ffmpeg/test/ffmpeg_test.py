...
def fixed(omnitime=6,endmargin=20,omniposition='(W-w)/2,(H-h)/2',logotext=False,tailaudio='mizmar.mp3'):
 ...
 g.overlay((l.adddestdir('videologo_s.png')),position='W-w,h',begintime=2.0,duration=duration-2) #top right logo
 if logotext: # position 5 initial logo
  g.overlay(u.logotext(size=0.3),0)
...
#   g.overlay(l.adddestdir('contact.gif'),begintime=i,position=re.sub(r'h\)/2',r'h)*2/3',re.sub(r'\(W-w\)/2',r'\(W-w\)',l.filter['overlay']['up'])),duration=10)
   ...
   g.overlay((l.adddestdir('cracker.mp3'),0.25),begintime=i) # music sync with omni text

 g.overlay(u.text2image(r'ThankYou::w:shade6\n::::40\nvisit:0.2:\nyoutube.com/c/minhinc:0.7:gi:shade10',margin=20,backcolor=(0,0,0,28)),begintime=duration-endmargin,position=re.sub(r't/\d',r't/4',l.filter['overlay']['up']),duration=endmargin/3) #end tail presentation
 g.overlay(u.text2image(r'Training Contacts:0.5:white\nWhatsApp +91 9483160610:0.7:gi:shade12',margin=20,backcolor=(0,0,0,28)),begintime=duration-endmargin*2/3,position=re.sub(r't/\d',r't/4',l.filter['overlay']['up']),duration=endmargin*2/3) #end tail presentation
...
fixed(logotext=True,omniposition=co(56),tailaudio='guitar_0.mp3',endmargin=30)
bannerlist=[]
for count,i in enumerate(getslotstamp(int(duration/180),begintime=0)):
 if i:
  bannerlist.append((i,(co(85),co(25))[count%2],'',title))
print(f'{bannerlist=}')
