...
class gifc:
 ...
 def videoaudioadd(self,*videoproperty):
  ...
  for i in videoproperty:
   print(f'videoaudioadd i={i}')
   if type(i[0])==tuple:
    i=((self.utili.image2gif(i[0][0],filtername=i[1][0],duration=float(re.split(r'-',i[1][1])[1])-float(re.split(r'-',i[1][1])[0])),i[0][1]),i[1][1])
   self.beginstring+=''.join([' -ss '+(' -to '.join(re.split('-',x)))+' -i '+re.sub(r'^=','',(i[0][0] if type(i[0])==tuple else i[0]))+' -ss '+(' -to '.join(re.split('-',x)))+' -i '+re.sub(r'^=?(.*)[.].*$',r'\1'+r'.mp3',(i[0][1] if type(i[0])==tuple else i[0]))+' ' for x in (i[1] if type(i[1])==tuple else [i[1]])]) if len(i)>1 else ' -i '+re.sub(r'^=','',i[0])+' -i '+re.sub(r'^=?(.*)[.].*$',r'\1'+r'.mp3',i[0])+' '
   concatstring+=''.join([('[io' if (str(self.libi.videowidth),str(self.libi.videoheight))!=self.libi.dimension(re.sub(r'^=','',(i[0][0] if type(i[0])==tuple else i[0]))) else '[')+str(x+count)+('' if (str(self.libi.videowidth),str(self.libi.videoheight))!=self.libi.dimension(re.sub(r'^=','',(i[0][0] if type(i[0])==tuple else i[0]))) else ':v')+']['+str(x+count+1)+':a]' for x in range(len(i[1] if type(i[1])==tuple else [i[1]]) if len(i)>1 else 1)])
   self.returnstring+=''.join(['['+str(x+count)+':v]'+self.utili.scalepad((i[0][0] if type(i[0])==tuple else i[0]),targetdimension=(self.libi.videowidth,self.libi.videoheight),getscalestr=True)+'[io'+str(x+count)+'];' for x in range(len(i[1] if type(i[1])==tuple else [i[1]]) if len(i)>1 else 1)]) if not re.search(r'^=',(i[0][0] if type(i[0])==tuple else i[0])) and (str(self.libi.videowidth),str(self.libi.videoheight))!=self.libi.dimension((i[0][0] if type(i[0])==tuple else i[0])) else ''
   count+=len(i[1] if type(i[1])==tuple else [i[1]])*2 if len(i)>1 else 2
  self.returnstring+=concatstring+'concat=n='+str(len(re.findall(' -i ',self.beginstring))/2)+':v=1:a=1[io'+str(len(re.findall(' -i ',self.beginstring))-1)+'][aout];' if len(re.findall(' -i ',self.beginstring))>1 else ''
