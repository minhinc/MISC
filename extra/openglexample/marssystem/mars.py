...
#  super().__init__(**dict({'objfile':'sphere.obj','material_diffuse':{'__UNKNOWN__':'mars.jpg'},'extratransformation':[[25,0,0,1]],'material_ambient':(0.3,0.3,0.3)},**kwarg))
  ...
  self.innerchildren.append(shader(objfile=None))
  ...
  self.children.append(shader(objfile='ring.obj',material_shininess=-1,material_diffuse={'__UNKNOWN__':(1,0.8,1)},transformation=[[90,1,0,0],[-40,0,0,1]],fixtransformation=[['s',2,2,2]]))
  self.children.append(shader(objfile='rock1.obj',material_shininess=-1,fixtransformation=[[180,0,1,0],[1.4,0,0],['s',10,10,10]]))
  self.children.append(shader(objfile='rock2.obj',material_shininess=-1,fixtransformation=[[180,0,1,0],[4,0,0],['s',0.4,0.4,0.4]]))
  '''
  ...
  '''
  self.children.append(shader(objfile='satelite.obj',transformation=[[40,0,1,0]],fixtransformation=[[2,0,0],[180,0,0,1],[-10,1,0,0],['s',0.1,0.1,0.1]]))
  '''
...
#   self.extratransformation=[[25,rotationaxis[0],0,rotationaxis[1]]]+[x for x in self.extratransformation if x[2]]
 ...
 def keyboard(self,**kwarg):
  ...
  if not hasattr(self,'extratransformation'):
   ...
   self.fixtransformation=[x for x in self.fixtransformation if len(x)==3 or x[0]=='s']+[[25,shader.utili.settlematrix(self.transformation)[:,0][2],0,shader.utili.settlematrix(self.transformation)[:,2][2]]] if type(shader.utili.settlematrix(self.transformation))==np.matrix else self.fixtransformation
