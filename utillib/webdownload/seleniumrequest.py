...
class _seleniumrequestc:
 ...
 def __init__(self,**kwarg):
  print(f'>< selenium.request.__init__ {self.driverdc=}')
#   traceback.print_stack()
 ...
 def gethtml(self,urls_,engine_='lynx',**kwarg):
  '''fetch html formatting code from urls_ using engine engine_
urls_(s) url of the webpage
engine_(s) engine to be used to fetch,ie. chrome lynx
<-(s) return string html data'''
  if re.search(r'chrome',engine_,flags=re.I):
   if not self.driver:
    self.driver=webdriver.Chrome()
   self.scrollpage()
   self.driver.get(urls_)
  return os.popen('lynx -source '+urls_).read()
...
#  print(f'D seleniumrequest.getlinkdata {urld=}')
 ...
 def scrollpage(self):
  while True:
   ...
   [self.driver.execute_script("window.scrollTo(0, document.documentElement.scrollHeight*"+str(float(i/5))+");") for i in range(1,6) if not time.sleep(0.5)]
   ht=self.driver.execute_script("return document.documentElement.scrollHeight;")
   if prev_ht==ht:
    break

 def googlelink(self,searchtext_,pagecount_): #takes company name
