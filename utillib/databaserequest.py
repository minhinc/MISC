...
#  print(f'linkupdatebulk {j=}')
  self.db.search2('linkvisited',*[(self.mutelink(x),re.sub('-','',datetime.date.today().isoformat())) for x in j],mode='fillbulk')
 def linkupdate(self,j,checkupdate=True):
  if not self.db.search2('linkvisited','name','=',self.mutelink(j),mode='search'):
   self.db.search2('linkvisited',self.mutelink(j),re.sub('-','',datetime.date.today().isoformat()),mode='fill')
