...
#data in html
 ...
 # print(f'databasec.reconnect {self.conn=} {self.cc=}')
  ...
  if (int(time.time())-databasec.reconnect.lastconnecttime) > 600: # 5 minutes connection works
   ...
   return False
  try:
   if re.search(r'raspberrypi',os.popen('uanme -a').read(),flags=re.I):
    self.conn=mysqlite3.connect(re.split(r'\s+',re.split('\n',open(os.path.expanduser('~/passwd')).read())[0])[3])
   else:
...
import MISC.utillib.util;MISC.utillib.util.cmc(__name__,MISC.utillib.util.mce(__name__,databasec) or databasec(False))
